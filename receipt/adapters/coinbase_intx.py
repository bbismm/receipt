...
class CoinbaseAdapter:
    ...
    def fetch_position(self, symbol: str) -> dict:
        """Best-effort current position. Falls back to deriving from recent fills
        if the venue product is not exposed via the perpetuals positions endpoint
        on this account type."""
        # Try INTX perpetuals positions if the SDK supports it
        try:
            portfolios = self._client.get_portfolios()
            pdict = portfolios.to_dict() if hasattr(portfolios, "to_dict") else dict(portfolios)
            for p in pdict.get("portfolios", []):
                if p.get("type") == "INTX":
                    pos = self._client.get_intx_position(
                        portfolio_uuid=p["uuid"], symbol=symbol
                    )
                    return self._normalize_position(
                        pos.to_dict() if hasattr(pos, "to_dict") else dict(pos),
                        symbol,
                    )
        except Exception:
            pass

        # Fallback: derive from net fills
        return self._derive_position_from_fills(symbol)

    def fetch_balance(self) -> dict:
        """Total + available USD across accounts."""
    ...
    def _derive_position_from_fills(self, symbol: str) -> dict:
        """Net buy − sell over recent fills. Slow but works for any account type."""
        try:
            fills = self._client.list_fills(product_id=symbol, limit=100)
